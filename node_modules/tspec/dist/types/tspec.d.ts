...
export declare namespace Tspec {
    type PathUrl = `/${string}`;
    export type HttpMethod = 'get' | 'post' | 'put' | 'patch' | 'delete' | 'options' | 'head';
    type PathParamValue = string | number;
    export type PathParam = {
        [key: string]: PathParamValue;
    };
    ...
    export type HeaderParam = {
        [name: string]: HeaderParamValue;
    };
    type CookieParamValue = string | number;
    export type CookieParam = {
        [name: string]: CookieParamValue;
    };
    export interface ApiSpecBase<Res extends any = any, P extends PathParam = PathParam, Q extends QueryParam = QueryParam, H extends HeaderParam = HeaderParam, C extends CookieParam = CookieParam> {
        summary?: string;
        description?: string;
        tags?: string[];
        security?: string;
    ...
    export type ApiSpec<T extends ApiSpecInput> = T & {
        path: T['path'] extends {} ? T['path'] : T['handler'] extends ExpressHandler ? PathOf<T['handler']> : never;
        responses: T['responses'] extends {
            [code: number]: any;
        } ? T['responses'] : T['handler'] extends ExpressHandler ? {
            200: ResBodyOf<T['handler']>;
        } : never;
        body: T['body'] extends {} ? T['body'] : T['handler'] extends ExpressHandler ? BodyOf<T['handler']> : never;
        query: T['query'] extends {} ? T['query'] : T['handler'] extends ExpressHandler ? QueryOf<T['handler']> : never;
        __handler: T['handler'];
    };
    type Path = {
        [method in HttpMethod]?: ApiSpecInput;
    };
    type Paths = {
        [path: PathUrl]: Path;
    };
    ...
    type WithBasePath<Base extends string | undefined, U extends string> = Base extends string ? `${Base}${U}` : U;
    type ParsePathKeys<U extends string> = U extends `${string}/{${infer P}}${infer R}` ? R extends string ? P | ParsePathKeys<R> : P : never;
    export type DefineApiSpec<T extends Controller<{
        [P in Extract<keyof T['paths'], PathUrl>]: {
            [M in HttpMethod]?: Omit<ApiSpecInput, 'path'> & {
                path?: {
                    [key in ParsePathKeys<WithBasePath<T['basePath'], P>>]: PathParamValue;
                };
                handler?: ExpressHandler;
            };
        };
        ...
        [P in Extract<keyof T['paths'], PathUrl>]: {
            [M in Extract<keyof T['paths'][P], HttpMethod>]: T['paths'][P][M] extends ApiSpecInput ? (Omit<ApiSpec<T['paths'][P][M]>, 'tags' | 'security'> & {
                method: M;
                url: WithBasePath<T['basePath'], P>;
                security: T['paths'][P][M]['security'] extends string ? T['paths'][P][M]['security'] : T['security'];
                tags: [
